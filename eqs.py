...
# cos(pi - acos(x)) = -x
# 2 * acos(cos_111) + 2 * acos(cos_11t) + 2 * acos(cos_r1t) + acos(cos_r1r) = 2 * pi
# acos(cos_111) + acos(cos_11t) + acos(cos_r1t) + acos(cos_r1r)/2 = pi
# acos(cos_111) + acos(cos_11t) + acos(cos_r1t) = pi - acos(cos_r1r)/2
# cos from both parts

F1 = (sum_of_cos(cos_111, cos_11t, cos_r1t) + half(cos_r1r))
F1 = sp.simplify(F1)

# 2 * acos(cos_1rr) + 2 * acos(cos_1rt) + 2 * acos(cos_srt) = 2 * pi
# acos(cos_1rr) + acos(cos_1rt) + acos(cos_srt) = pi

FR = (sum_of_cos(cos_1rr, cos_1rt, cos_srt) + 1)
FR = sp.simplify(FR)

FS = ((S ** 2 + R * S + T * S - R * T) / ((R + S) * (S + T)) - sp.Rational(1, 2))
FS = sp.simplify(FS)

# acos(cos_1t1) + 2 * acos(cos_1tr) + 2 * acos(cos_rts) = 2 * pi
# acos(cos_1tr) + acos(cos_rts) = pi - acos(cos_1t1)/2
